...
class Command(BaseCommand):
    help = 'Create 14 guides for all districts'
    ...
    def handle(self, *args, **options):
        ...
        with transaction.atomic():
            for data in guides_data:
                ...
                if created:
                    ...
                    self.stdout.write(self.style.WARNING(f'🔄 Updated user: {user.email}'))
                
                # Get district
                try:
                    district = District.objects.get(name=data['district'])
                except District.DoesNotExist:
                    self.stdout.write(self.style.ERROR(f'⚠️ District not found: {data["district"]}'))
                    continue
                ...
                guide, created = Guide.objects.get_or_create(
                    ...
                    defaults={
                        ...
                        'bio': f'Experienced local guide for {data["district"]} district.',
                ...
                if created:
                    created_count += 1
                    self.stdout.write(self.style.SUCCESS(f'✅ Created guide: {guide.full_name}'))
                else:
                    updated_count += 1
                    ...
                    guide.save()
                    self.stdout.write(self.style.WARNING(f'🔄 Updated guide: {guide.full_name}'))
                
                # Assign district
                guide.districts.clear()
                guide.districts.add(district)
                self.stdout.write(f'  📍 Assigned to: {district.name}')

        self.stdout.write("\n" + "="*50)
        self.stdout.write(self.style.SUCCESS("🎉 GUIDE CREATION COMPLETE!"))
        self.stdout.write("="*50)
        self.stdout.write(f"✅ Created: {created_count} guides")
        self.stdout.write(f"🔄 Updated: {updated_count} guides")
        self.stdout.write(f"📊 Total guides: {Guide.objects.count()}")
        self.stdout.write("="*50)
        ...
        for guide in Guide.objects.filter(is_active=True).order_by('full_name'):
            ...
            self.stdout.write(f"  🧑‍🏫 {guide.full_name} ({guide.email})")
            self.stdout.write(f"     📍 District: {districts[0] if districts else '⚠️ NONE!'}")
            self.stdout.write(f"     🔑 Password: punnolil")
