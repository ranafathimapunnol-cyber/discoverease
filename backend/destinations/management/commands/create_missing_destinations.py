# destinations/management/commands/create_missing_destinations.py
...
class Command(BaseCommand):
    help = 'Create destinations for places that don\'t have them'
    ...
    def handle(self, *args, **options):
        ...
        if total == 0:
            self.stdout.write('✅ All places have destinations!')
            return
        ...
        for place in places_without_dest:
            try:
                ...
                self.stdout.write(f'  ❌ Error creating "{place.name}": {e}')
                errors += 1
