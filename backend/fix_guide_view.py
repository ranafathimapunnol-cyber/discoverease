...
# Find and replace the problematic get_queryset method
old_pattern = r'def get_queryset\(self\):.*?(?=def |@action|$)'
...
new_method = '''    def get_queryset(self):
        ...
        if district:
            # Use icontains to filter by district name
...
if re.search(old_pattern, content, re.DOTALL):
    ...
    with open('guides/views.py', 'w') as f:
        f.write(content)
    print("✅ Fixed get_queryset method")
else:
    print("❌ Could not find the method")
    # Try alternative pattern
    old_pattern2 = r'def get_queryset\(self\):.*?return queryset'
    if re.search(old_pattern2, content, re.DOTALL):
        content = re.sub(old_pattern2, new_method, content, flags=re.DOTALL)
        with open('guides/views.py', 'w') as f:
            f.write(content)
        print("✅ Fixed get_queryset method (alternative)")
    else:
        print("❌ Could not find the method. Please manually update.")
