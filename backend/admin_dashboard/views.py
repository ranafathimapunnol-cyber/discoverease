...
# - Guide update with PUT and PATCH support
# - Profile picture upload endpoint
# - Guide delete endpoint
# - All proper URL routing with unique paths
...
class AdminViewSet(viewsets.ViewSet):
    ...
    # ============================================
    # GET /api/admin/users/touristers/
    # ============================================
    ...
    def toggle_user_status(self, request, pk=None):
        ...
        try:
            ...
            if user.id == request.user.id:
                return Response({'success': False, 'error': 'Cannot change your own status'}, status=400)
        ...
        try:
            ...
            for g in guides:
                data.append({
                    ...
                    'profile_image': g.profile_image.url if g.profile_image else None,
                    ...
                    'price_per_day': float(g.price_per_day) if g.price_per_day else 0,
                    'price_per_hour': float(g.price_per_hour) if g.price_per_hour else 0,
                    'rating': float(g.rating) if hasattr(g, 'rating') else 0,
                    'booking_count': 0,
            ...
            return Response({
                ...
                'guides': data,
                'count': len(data),
                'active_count': guides.filter(is_active=True).count(),
                'verified_count': guides.filter(is_verified=True).count()
            })
        except Exception as e:
    ...
    # ============================================
    # ✅ FIXED: POST /api/admin/guides/{id}/upload-profile-pic/
    # ============================================
    @action(detail=True, methods=['post'], url_path='upload-profile-pic')
    def upload_guide_profile_pic(self, request, pk=None):
        """Upload profile picture - URL: /api/admin/guides/{id}/upload-profile-pic/"""
        if not self._check_admin_access(request):
            return Response({'error': 'Admin access required'}, status=403)
        try:
            from guides.models import Guide
    ...
    # ============================================
    # ✅ FIXED: DELETE /api/admin/guides/{id}/delete/
    # ============================================
    @action(detail=True, methods=['delete'], url_path='delete')
    def delete_guide(self, request, pk=None):
        """Delete a guide - URL: /api/admin/guides/{id}/delete/"""
        if not self._check_admin_access(request):
            return Response({'error': 'Admin access required'}, status=403)
        try:
            from guides.models import Guide
    ...
    def verify_guide(self, request, pk=None):
        ...
        try:
            ...
            return Response({'success': True, 'message': 'Guide verified successfully'})
        except Exception as e:
